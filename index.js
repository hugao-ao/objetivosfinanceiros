...
<head>
  ...
  <title>Controle Financeiro V8 - Modificado</title> <!-- Versão atualizada -->
  <style>
    body {
    ...
    .container-principal {
      display: flex;
      width: 300%; /* Ajustado para 3 seções: Lançamentos, Resumo, Mensagens */
      transition: transform 0.5s ease;
    }

    .container-resumo-ativo {
      transform: translateX(-33.33%); /* Mostra Resumo */
    }

    .container-mensagens-ativo {
      transform: translateX(-66.66%); /* Mostra Mensagens */
    }

    /* Seções */
    .secao-lancamentos,
    .secao-resumo,
    .secao-mensagens { /* Nova seção */
      width: 33.33%; /* Cada seção ocupa 1/3 */
      padding: 0 20px;
...
// *** INÍCIO MODIFICAÇÃO: Função para editar dia de fechamento manualmente ***
    ...
    function gerarMensagens() {
      ...
      cartoes.forEach(cartao => {
        ...
        const textoVencimento = `Vencimento da fatura do cartão ${cartao.nome}`;
		...
		dataVencimentoNoMes.setHours(0, 0, 0, 0)			 if (dataVencimentoNoMes >= inicioMesNorm && dataVencimentoNoMes <= fimMesNorm) {
    ...
    function gerarInsightsFinanceiros(dados) {
      ...
      // Insight 3: Maiores Categorias de Despesa (Anual)
      const despesasCategorizadas = [
        { nome: "Despesas Fixas", valor: dados.totalFixas },
        { nome: "Despesas Variáveis", valor: dados.totalVariaveis },
      ].sort((a, b) => b.valor - a.valor);

      if (dados.totalDespesas > 0) {
        insightsHTML += `<li>Anualmente, suas maiores categorias de despesa são: <strong>${despesasCategorizadas[0].nome}</strong> (${formatarMoeda(despesasCategorizadas[0].valor)})${despesasCategorizadas.length > 1 ? ` e <strong>${despesasCategorizadas[1].nome}</strong> (${formatarMoeda(despesasCategorizadas[1].valor)})` : ''}.</li>`;
      }
